...
namespace gdasapp {
  ...
  class wodT2Ioda : public NetCDFToIodaConverter {
   public:
    explicit wodT2Ioda(const eckit::Configuration & fullConfig, const eckit::mpi::Comm & comm)
    ...
    gdasapp::obsproc::iodavars::IodaVars providerToIodaVars(const std::string fileName) final {
      ...
      int dimT   = ncFile.getDim("Temperature_obs").getSize();
      ...
      // Read Temperature_row_size
      std::vector<int> tempRowSize(dimLon);
      netCDF::NcVar tempRowSizeVar = ncFile.getVar("Temperature_row_size");
      ...
      netCDF::NcVar tObsVar = ncFile.getVar("Temperature");
