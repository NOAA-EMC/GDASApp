...
namespace gdasapp {
  ...
  class RTOFSInSitu : public NetCDFToIodaConverter {
   public:
    explicit RTOFSInSitu(const eckit::Configuration & fullConfig, const eckit::mpi::Comm & comm)
    ...
    gdasapp::obsproc::iodavars::IodaVars providerToIodaVars(const std::string fileName) final {
      oops::Log::info() << "Processing files provided by RTOFS" << std::endl;
...
*/
      ...
      std::vector<std::string> intMetadataNames = {"dummyBasin"};
