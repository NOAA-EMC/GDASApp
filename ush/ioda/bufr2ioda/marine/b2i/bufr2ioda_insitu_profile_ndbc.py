...
platform_description = 'NDBC profiles from mbuoyb: temperature and salinity'


class NDBCConfig(Bufr2iodaConfig):
    def ioda_filename(self):
        return f"{self.cycle_type}.t{self.hh}z.insitu_profile_ndbc.{self.cycle_datetime}.nc4"


class NDBCIODAVariables(MbuoybTropicalIODAVariables):
    def filter(self):
        super().filter()
        mask = [False if int(rpid) in PIRATA or int(rpid) in RAMA or int(rpid) in TAO_TRITON else True for rpid in self.metadata.stationID]
