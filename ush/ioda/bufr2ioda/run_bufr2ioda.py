...
@logit(logger)
def bufr2ioda(current_cycle, RUN, DMPDIR, config_template_dir, COMIN_OBS):
    logger.info(f"Process {current_cycle} {RUN} from {DMPDIR} to {COMIN_OBS} using {config_template_dir}")
    ...
    os.makedirs(COMIN_OBS, exist_ok=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument('COMIN_OBS', type=Path, help='path to output ioda format dump files')
    args = parser.parse_args()
    bufr2ioda(args.current_cycle, args.RUN, args.DMPDIR, args.config_template_dir, args.COMIN_OBS)
