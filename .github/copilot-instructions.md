...
- Clone and set up the main JCB repository with clients:
  ...
  ```
- The init script clones repositories containing the clients (jcb-gdas, jcb-rdas, etc.) and algorithms into `src/jcb/configuration/`
...
- The JCB-GDAS client uses GitHub Actions for CI testing at `.github/workflows/run_jcb_basic_testing.yaml`
...
#### Client Directory Structure (for reference)
```
...
- This is a configuration client - no compilation/build process required
